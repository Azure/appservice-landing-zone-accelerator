...
#####################################
variable "hub_remote_state_settings" {
  type = object({
  ...
  description = "The settings for the hub remote state."
...
variable "hub_vnet_settings" {
  ...
  validation {
    condition     = var.hub_vnet_settings != null ? (var.hub_vnet_settings.rg_name != null && var.hub_vnet_settings.vnet_name != null) : true
    error_message = "Either `hub_remote_state_settings` or hub_vnet_settings` must be provided. If hub_vnet_settings is not null, `rg_name` and `vnet_name` attributes of the hub vnet must both be provided."
  }
}
...
variable "vm_entra_admin_object_id" {
  type        = string
  description = "The Azure AD object ID for the VM admin user/group. If vm_entra_admin_username is not specified, this value will be used."
  default     = null
}
