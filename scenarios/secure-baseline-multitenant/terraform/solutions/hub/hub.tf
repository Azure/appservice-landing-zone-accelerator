...
module "bastion" {
  ...
  subnet_id      = module.network.subnets[index(module.network.subnets.*.name, var.bastion_subnet_name)].id
  resource_group = azurerm_resource_group.hub.name
  location       = azurerm_resource_group.hub.location
...
module "firewall" {
  ...
  # Retrieve the subnet id by a lookup on subnet name from the list of subnets in the module output
  subnet_id      = module.network.subnets[index(module.network.subnets.*.name, var.firewall_subnet_name)].id
  resource_group = azurerm_resource_group.hub.name
  location       = azurerm_resource_group.hub.location
