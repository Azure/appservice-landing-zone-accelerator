...
module webApp 'br/public:avm/res/web/site:0.3.8' = {
  ...
  params: {
    ...
    httpsOnly: httpsOnly
    appServiceEnvironmentResourceId: !empty(appServiceEnvironmentId) ? appServiceEnvironmentId: null
    storageAccountRequired: storageAccountRequired
    keyVaultAccessIdentityResourceId: !empty(keyVaultAccessIdentityResourceId) ? keyVaultAccessIdentityResourceId : null
    ...
    appInsightResourceId: appInsightId
    appSettingsKeyValuePairs: appSettingsKeyValuePairs
    privateEndpoints: privateEndpoints
    slots: [for (slot, index) in slots: {
        name: slot.name
        ...
        managedIdentities: contains(slot, 'systemAssignedIdentity') ? slot.systemAssignedIdentity : systemAssignedIdentity
        ...
        diagnosticSettings: (!empty(diagnosticWorkspaceId)) ? [
          {
            workspaceResourceId: empty(diagnosticWorkspaceId) ? null : diagnosticWorkspaceId
          }
          {
...
output slots array = [for (slot, index) in slots: webApp.outputs.slots[index].name]

@description('The list of the slot resource ids.')
output slotResourceIds array = [for (slot, index) in slots:  webApp.outputs.slotResourceIds[index]]

@description('The principal ID of the system assigned identity.')
output systemAssignedPrincipalId string = systemAssignedIdentity && contains(webApp.outputs.systemAssignedMIPrincipalId, 'principalId') ? webApp.outputs.systemAssignedMIPrincipalId : ''


@description('The principal ID of the system assigned identity of slots.')
output slotSystemAssignedPrincipalIds array = [for (slot, index) in slots: webApp.outputs.slotSystemAssignedMIPrincipalIds[index]]
